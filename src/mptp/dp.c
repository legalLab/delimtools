...
static void backtrack(rtree_t * node, int index, bool *warning_minbr)
{
...
/* ── rmptp extension ────────────────────────────────────────────────────────
 * dp_get_stats(): called by rmptp_wrap.c after dp_ptp() has run.
 * Re-derives best_logl, pvalue and lrt_pass from the filled DP table so
 * that the wrapper can return them to R without parsing any output file.
 * ----------------------------------------------------------------------- */
void dp_get_stats(rtree_t * tree,
                  long      method,
                  double  * best_logl_out,
                  double  * pvalue_out,
                  int     * lrt_pass_out,
                  unsigned int * n_species_out)
{
  int i;
  int best_index = 0;
  double max = 0;
  ...
  if (method == PTP_METHOD_MULTI)
  {
    double min_aic = aic(vec[0].score_multi, vec[0].species_count,
                         tree->leaves + 2);
    for (i = 1; i < tree->edge_count; i++)
    {
      if (vec[i].filled)
      {
        double a = aic(vec[i].score_multi, vec[i].species_count,
                       tree->leaves + 2);
        if (a < min_aic) { min_aic = a; best_index = i; }
      }
    }
    max = vec[best_index].score_multi;
  }
  else
  {
    max = vec[0].score_single;
    for (i = 1; i < tree->edge_count; i++)
      if (vec[i].filled && vec[i].score_single > max)
      { max = vec[i].score_single; best_index = i; }
  }
  ...
  double pv = -1;
  int lrt_pass = lrt(tree->coal_logl, max, df, &pv);

  *best_logl_out  = max;
  *pvalue_out     = pv;
  *lrt_pass_out   = lrt_pass;
  *n_species_out  = vec[best_index].species_count;
}
