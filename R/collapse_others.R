...
collapse_others <- function(data, hap_tbl, identifier, cols){
  ...
  collapse_tbl <- purrr::map(cols,
                             ~{ hap_tbl %>%
                                 ...
                                 dplyr::mutate(x= dplyr::pull(data, .x)[match(haplotype, dplyr::pull(data, {{ identifier }}))],
                                               y= dplyr::pull(data, .x)[match(collapsed, dplyr::pull(data, {{ identifier }}))]) %>%
                                 dplyr::mutate("collapsed_{.x}" := dplyr::if_else(is.na(y),
                                                                                  x,
                                                                                  paste(unique(c(x, y)), collapse = ", ")), .by = haplotype) %>%
                                 dplyr::select(c(haplotype, n_seqs, tidyselect::ends_with(cols))) %>%
                                 dplyr::distinct(haplotype, .keep_all = TRUE) %>%
                                 dplyr::full_join(hap_tbl, ., by= c("haplotype", "n_seqs"))}) %>%
    purrr::reduce(., dplyr::full_join, by= c("haplotype", "n_seqs", "collapsed"))
