...
delim_autoplot <- function(delim, tr, consensus=TRUE, n_match= NULL,
  ...
  if(consensus == TRUE) {
    ...
    delim_tile <- ggplot2::ggplot(delim_long, aes(x= method, y= labels, color= spp, fill= spp))+
      ggplot2::geom_tile(data= subset(delim_long, !is.na(spp)), width= 0.5,show.legend = FALSE)+
