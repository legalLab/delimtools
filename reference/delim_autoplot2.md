...
`delim_autoplot2()` returns a phylogenetic tree plotted using `ggtree`
...
- species:
  ...
  column name in `tbl_labs` which contains species names for each tip of
  the tree.
...
`delim_autoplot2()` is a wrapper for tree plotting with associated data
...
consensus is reached, an "X" will be plotted instead. This function is a
modified version of [delim_autoplot](delim_autoplot.md) which plots
...
#> Warning: Argument `delim_order` not provided. Using default order from `delim`.
#> Warning: ⚠ `n_match` was not found. Using `n_match= 5` instead.
p
