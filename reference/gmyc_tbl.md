...
`gmyc_tbl()` processes output from
[gmyc](https://rdrr.io/pkg/splits/man/gmyc.html) into an object of class
...
- gmyc_res:
  ...
  Output from [gmyc](https://rdrr.io/pkg/splits/man/gmyc.html).
...
`splits` package uses [gmyc](https://rdrr.io/pkg/splits/man/gmyc.html)
to optimize genetic clusters and
[spec.list](https://rdrr.io/pkg/splits/man/gmyc.html) to cluster samples
into species partitions. `gmyc_tbl()` turns these results into a tibble
which matches the output from [bgmyc_tbl](bgmyc_tbl.md) and
